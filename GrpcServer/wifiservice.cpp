...
::grpc::Status SystemWifiConfigService::SendSystemWifiConfig(::grpc::ServerContext* context, const ::ConfigWifi* request, ::ConfigWifi* response)
{
    ...
    sprintf(cmd,"ip link set %s down","wlx7cc2c62577c4");
	system(cmd);
    std::cout<<cmd<<std::endl;
    if(request->ip().length() >0){
        std::string d = "ifconfig wlx7cc2c62577c4 " + request->ip() + " netmask " + request->mask();
        std::cout<<d<<std::endl;
	    system(d.c_str());	 
    }

    if(request->gateway().length()>0){
        std::string d  = "route add default gw " + request->gateway() + " wlx7cc2c62577c4";
        std::cout<<d<<std::endl;
	    system(d.c_str()); 
    }
    sprintf(cmd,"ip link set %s up","wlx7cc2c62577c4");
	system(cmd);
