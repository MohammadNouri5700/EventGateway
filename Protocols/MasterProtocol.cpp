...
void ProtocolS::ModBus::MasterProtocol::Create(Connection *Conn)
{
    ...
    {
        //   std::cout << "Reads input registers of slave[" << slv->number() << "] on " << Node.connection() << " (" << Node.settings() << ")" << std::endl;
        //             Node.setResponseTimeout(3);
        //             if (Node.open())
        //             { // open a connection
        ...
        //                 else
        //                 {
        //                     std::cerr << "Unable to read input registers ! " << Node.lastError() << std::endl;
        //                 }
        //                 Node.close();
        //             }
        //             else
        //             {
        //                 std::cerr << "Unable to open MODBUS connection to "
        //                           << " : " << Node.lastError() << std::endl;
        //                 exit(EXIT_FAILURE);
        //             }
        while (slv->isOpen() &&!bStopThread)
        {
            ...
            for (auto it = Observer.begin(); it != Observer.end(); it++)
            {
                ...
                {
                    int b = slv->readInputRegister(1, int_values[0]);
                    if (b == size)
                    {
                        tag->setValue(int_values, size * sizeof(uint16_t));
                        int a = 0;
