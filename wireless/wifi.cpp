...
// {
//     system("sudo iwlist  wlx7cc2c62577c4 scan | egrep 'ESSID' > wifiessid.tmp") ;
//     system("sudo iwlist  wlx7cc2c62577c4 scan | egrep 'Quality' > wifiquality.tmp") ;
//     std::ifstream infileEssid("wifiessid.tmp");
//     std::ifstream infileQuality("wifiquality.tmp");
//     std::string Signal, Essid;
//     while (infileEssid >>  Essid ,infileQuality >>  Signal )
//     {
...
//          int locQuality = Signal.find("level=")+1;
//         if (locQuality > 0)
//         {
//             std::string tokenQuality = Signal.substr(locQuality+5,   4 ); 
...
std::map<std::string,networks> wifi::SystemScan(void)
{
    system("sudo iwlist  wlx7cc2c62577c4 scan | egrep 'ESSID|Quality' > wifiessid.tmp") ;
    std::string Essid;
    std::string line;
    ifstream myfile;
...
//     std::cout<< "Interface *************"<< wreq.ifr_name<<std::endl;
...
std::string wifi::Connect(std::string data)
{
    ...
    if(Pass.length() > 0){
        ...
        Command= "sudo nmcli d wifi connect "+Essid+" password "+Pass+" &> tempcommand.txt";
        std::cout << Command << std::endl;
    }else{
         std::cout << "Essid = " << Essid << " " << "Pass = " << Pass << std::endl;
        Command= "sudo nmcli d wifi connect "+Essid +" &> tempcommand.txt";
        std::cout << Command << std::endl;
    }
    system(Command.c_str()) ;
